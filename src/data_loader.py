...
def get_summary(df, date_col, sales_col):
    """
    الحين يقبل أي اسم عمود مو بس Walmart
    """
    summary = {
    ...
    # إذا في عمود متجر/فرع
    for col in df.columns:
        if df[col].dtype in ['int64', 'object'] and col not in [date_col, sales_col]:
            if df[col].nunique() < 100:
                try:
                    grouped = df.groupby(col)[sales_col].sum()
                    if len(grouped) > 0 and grouped.sum() > 0:
                        summary['group_col'] = col
                        summary['num_groups'] = df[col].nunique()
                        summary['best_group'] = str(df.groupby(col)[sales_col].sum().idxmax())
                        summary['worst_group'] = str(df.groupby(col)[sales_col].sum().idxmin())
                        break
                except Exception:
                    continue    
